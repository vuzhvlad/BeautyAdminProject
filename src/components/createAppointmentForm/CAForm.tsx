...
function CAForm() {
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    createNewAppointment(formData)
      .then(() => {
        ...
        setFormData({
          //setting form epty after submiting
          name: "",
          service: "",
          phone: "",
          date: "",
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const { name, value } = e.target; // getting value and name from the input

    setFormData((prevData) => ({
      // changing those value every time, and leaving prevData for imutability
      ...prevData,
      [name]: value,
    }));
  };
